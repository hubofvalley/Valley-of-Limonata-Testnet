...
| **1d. Show Node Status** | Reads the local RPC port from `~/.limonatad/config/config.toml`, prints the pretty JSON from `curl http://127.0.0.1:<port>/status \| jq`, and compares local height with network latest height from Limonata EVM RPC (`https://rpc.limonata.xyz`). `catching_up: false` = fully synced. | Check sync progress; prerequisite check before creating a validator. |
